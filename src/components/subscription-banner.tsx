...
export function SubscriptionBanner() {
  ...
  // User has an active subscription → show nothing (they're good)
  ...
  if (trial?.isActive && trial.daysRemaining > 0) {
    ...
    return (
      <div
        className={`
          relative px-4 py-2.5 border-b text-sm flex items-center gap-3 flex-wrap
          ${
            isUrgent
      ...
      >
          ...
          <span className="text-xs sm:text-sm truncate">
            {isUrgent ? (
              <strong className="font-semibold">
                ⚠️ Your trial ends in {trial.daysRemaining} day{trial.daysRemaining !== 1 ? "s" : ""}!
              </strong>
            ) : (
              <>
            ...
            <span className="hidden sm:inline text-muted-foreground ml-1">
              {isUrgent
                ? "Subscribe to keep your data active."
                : `We recommend the ${tierName} plan.`}
            </span>
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Badge
            ...
            className={`
              text-[10px] px-2 py-0
              ${isUrgent ? "border-amber-500/30 text-amber-600 dark:text-amber-400" : "border-primary/30 text-primary"}
            `}
          >
            {trial.daysRemaining}d left
          </Badge>
          <Link href="/pricing">
            <Button size="sm" variant={isUrgent ? "default" : "outline"} className="h-7 text-xs px-3">
  ...
  // ─── Trial just expired (isExpired flag) ────────────────────────────
  if (trial?.isExpired || trial?.daysRemaining === 0) {
    ...
    return (
      <div className="relative px-4 py-2.5 border-b border-amber-500/20 bg-amber-500/10 text-sm flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <AlertTriangle className="h-4 w-4 shrink-0 text-amber-500" />
          <span className="text-xs sm:text-sm truncate text-amber-700 dark:text-amber-300">
            <strong>Your free trial has ended.</strong>{" "}
            <span className="text-muted-foreground">
              Subscribe to keep your pipeline, automations, and client data active.
            </span>
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
  ...
  // ─── No active trial and no subscription → payment needed ───────────
    ...
    return (
      <div className="relative px-4 py-2.5 border-b border-primary/10 bg-primary/5 text-sm flex items-center gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-xs sm:text-sm truncate">
            Unlock full access to Mapato.{" "}
            <span className="text-muted-foreground hidden sm:inline">
              Pay via M-Pesa, Mobile Money, or Visa/Mastercard.
            </span>
          </span>
        </div>
        <div className="flex items-center gap-2 shrink-0">
